...
test("displays images from the server", async () => {
  ...
  const hiddenLoadingSpinner = screen.getByRole("progressbar", {
    hidden: true,
  });
  expect(hiddenLoadingSpinner).not.toBeVisible();

  // alternative to above
  const altHiddenLoadingSpinner = screen.queryByRole("progressbar");
  expect(altHiddenLoadingSpinner).not.toBeInTheDocument();
